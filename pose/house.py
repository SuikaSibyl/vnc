"""house.py – single-axis (Y) differentiable pose optimisation for the house scene.

This follows the same structure and outputs as rubiks.py, but optimises only
vertical-axis rotation and defaults to the house GLB scene.
"""
...
# ---------------------------------------------------------------------------
class House:
    """Wraps the house GLB with a differentiable Y rotation parameter."""
...
# ---------------------------------------------------------------------------
def render_smooth_house(theta_p: torch.Tensor, update_fn, context_args: dict):
...
def _get_video_reference_frame(
    method: str,
    iteration: int,
    video_scheduler: VideoFrameScheduler,
    gt_img_torch: torch.Tensor,
    ...
    ref_np = gt_img_torch[0, ..., :3].cpu().numpy()
    ref_np = _replace_white_bg(ref_np)
    return ref_np, False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Differentiable single-axis (Y) pose optimisation for the house scene."
    )
    parser.add_argument(
        "--input",
        default=str(SCENES_ROOT / "house" / "fantasy_house.glb"),
    )
    parser.add_argument(
        "--output-dir",
        default=str(OUTPUTS_ROOT / "house"),
    )
    ...
    parser.add_argument("--video-path", type=str, default=str(SCENES_ROOT / "house" / "videos" / "output.mp4"))
    ...
    for method in args.methods:
        ...
        if method == "prdpt":
            ...
            smooth_render = smoothFn(render_smooth_house, context_args=None, device=device)
        ...
        for iteration in pbar:
            ...
            if args.save_trajectory_video:
                ...
                if method in {"video_nvdiffrast", "video_pytorch3d"}:
                    ref_np, ref_flip = _get_video_reference_frame(
                        method=method,
                        iteration=iteration,
                        video_scheduler=video_scheduler,
                        gt_img_torch=gt_img_torch,
                    )
                    trajectory_ref_frames.append((_to_uint8_frame(ref_np, flip=ref_flip), iteration))

            deg_y = float(np.rad2deg(model.rotation_y.item()))
            dy = abs(((deg_y - args.target_deg) + 180.0) % 360.0 - 180.0)
            ...
            if (iteration + 1) % args.log_every == 0:
                ...
                if method in {"video_nvdiffrast", "video_pytorch3d"}:
                    ...
                    _save_png(out_dir / method_tag / f"iter_{iteration + 1:04d}_ref.png", ref_np, flip=ref_flip)
