"""castle.py – single-axis (Y) differentiable pose optimisation for the castle scene.

This follows the same structure and outputs as house.py, but defaults to the
sand_castle GLB scene.
"""
...
# ---------------------------------------------------------------------------
class Castle:
    """Wraps the castle GLB with a differentiable Y rotation parameter."""
    ...
    def load_scene(
        ...
        self._proj_view, self._center_t, _ = _build_camera(mesh, None, device)
...
def _composite_bg(img: torch.Tensor, bg_color: tuple = (0.961, 0.969, 0.980)) -> torch.Tensor:
    """Composite RGBA over a light gray background."""
...
def _replace_white_bg(img: np.ndarray, bg_color: tuple = (0.961, 0.969, 0.980)) -> np.ndarray:
    """Replace near-white pixels with a light gray for consistent visualization."""
...
def _gauss_downsample(img: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    ...
    k = kernel.expand(C, 1, kernel_h, kernel_w)
    x = torch.nn.functional.conv2d(x, k, padding=(kernel_h // 2, kernel_w // 2), groups=C)
    x = x[:, :, ::2, ::2]
...
def gaussian_pyramid_loss(
    ...
    radius = max(2, int(4.0 * blur_sigma + 0.5))
    coords = torch.arange(-radius, radius + 1, device=pred.device, dtype=pred.dtype)
    k1d = torch.exp(-0.5 * (coords / blur_sigma) ** 2)
    k1d = k1d / k1d.sum()
...
# ---------------------------------------------------------------------------
def render_smooth_castle(theta_p: torch.Tensor, update_fn, context_args: dict):
    ...
    with torch.no_grad():
        for i in range(theta_p.shape[0]):
            ...
            color = _render(
                ...
                angle, model._resolution, device,
            )
            img = color[0]
            if img.shape[-1] == 4:
                img = torch.cat([img[..., :3], torch.ones_like(img[..., 3:4])], dim=-1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Differentiable single-axis (Y) pose optimisation for the castle scene."
    )
    parser.add_argument(
        "--input",
        default=str(SCENES_ROOT / "castle" / "sand_castle.glb"),
    )
    parser.add_argument(
        "--output-dir",
        default=str(OUTPUTS_ROOT / "castle"),
    )
    ...
    for method in args.methods:
        ...
        if method == "prdpt":
            ...
            smooth_render = smoothFn(render_smooth_castle, context_args=None, device=device)
