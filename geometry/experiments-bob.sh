...
"$PYTHON_BIN" "$SCRIPT_DIR/optmizer.py" \
  ...
  --trajectory-fps 12 \
  --output-dir "$OUTPUT_ROOT/bob"

rm -rf "$OUTPUT_ROOT/bob_video"
...
"$PYTHON_BIN" "$SCRIPT_DIR/optmizer.py" \
  ...
  --save-trajectory-video \
  --trajectory-fps 12 \
  --output-dir "$OUTPUT_ROOT/bob_video"
...
methods = {}
for run_dir in (base_dir / "bob", base_dir / "bob_video"):
...
summary_path = base_dir / "bob_optimization_time_summary.json"
