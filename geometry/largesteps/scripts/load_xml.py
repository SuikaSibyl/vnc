...
def scale_matrix(s):
    """
    Builds a homogeneous coordinate scale matrix.

    Parameters
    ----------

    s : array-like of length 1 or 3
        Uniform scale (float) or per-axis scale [sx, sy, sz]
    """
    mat = torch.eye(4, device='cuda')
    vals = np.fromstring(s, dtype=float, sep=" ")
    if len(vals) == 1:
        mat[0, 0] = mat[1, 1] = mat[2, 2] = vals[0]
    else:
        mat[0, 0], mat[1, 1], mat[2, 2] = vals[0], vals[1], vals[2]
    return mat
    ...
    for plugin in root:
        if plugin.tag == "default":
            ...
            if plugin.attrib["type"] == "ply":
                mesh_path = None
                shape_mat = torch.eye(4, device='cuda')
                for prop in plugin:
                    if prop.tag == "string" and prop.attrib["name"] == "filename":
                        ...
                        for tr in prop:
                            if tr.tag == "rotate":
                                if "x" in tr.attrib:
                                    shape_mat = rotation_matrix("x", float(tr.attrib["angle"])) @ shape_mat
                                elif "y" in tr.attrib:
                                    shape_mat = rotation_matrix("y", float(tr.attrib["angle"])) @ shape_mat
                                else:
                                    shape_mat = rotation_matrix("z", float(tr.attrib["angle"])) @ shape_mat
                            elif tr.tag == "translate":
                                shape_mat = translation_matrix(np.fromstring(tr.attrib["value"], dtype=float, sep=" ")) @ shape_mat
                            elif tr.tag == "scale":
                                shape_mat = scale_matrix(tr.attrib["value"]) @ shape_mat
                            elif tr.tag == "matrix":
                                vals = np.fromstring(tr.attrib["value"], dtype=float, sep=" ").reshape(4, 4)
                                shape_mat = torch.tensor(vals, dtype=torch.float32, device='cuda') @ shape_mat
                if mesh_path is not None:
                    assert "id" in plugin.attrib.keys(), "Missing mesh id!"
                    mesh = read_ply(mesh_path)
                    if not torch.allclose(shape_mat, torch.eye(4, device='cuda')):
                        v = mesh["vertices"]
                        ones = torch.ones(v.shape[0], 1, device=v.device, dtype=v.dtype)
                        v_h = torch.cat([v, ones], dim=1)
                        mesh["vertices"] = (v_h @ shape_mat[:3, :].T)
                    scene_params[plugin.attrib["id"]] = mesh
            else:
